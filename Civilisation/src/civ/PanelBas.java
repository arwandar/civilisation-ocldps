...
public class PanelBas extends JPanel implements ActionListener {
	...
	JTextArea infoText;
	CardLayout cl;
	String[] enumCl = { "Archerie", "atelier de siège", "Caserne", "Ecurie",
			"Hotel de ville", "Mur", "Port", "mine", "Tour des mages",
			"Tourelle", "Archer", "Catapulte", "Cavalier archer", "Chevalier",
			"Galere", "Healer", "Magicien", "Milicien", "Peon", "Transporteur",
			"casevide" };
	JPanel[] jcard;
	...
	public PanelBas(int hauteur, int largeur, FntPrcpl bouh) {
		this.saFenetre = bouh;
		this.setLayout(null);
		this.setBounds(0, (hauteur - (30 * hauteur / 100)), largeur,
				(30 * hauteur / 100));
		...
		this.infoText.setBounds(this.getWidth() * 4 / 5, 0,
				this.getWidth() / 5, this.getHeight());
		this.add(this.infoText);

		// init panelactionpossibles
		this.panelActionsPossibles = new JPanel();
		this.panelActionsPossibles.setBounds(this.getWidth() * 1 / 5, 0,
				this.getWidth() * 3 / 5, this.getHeight());
	...
	public void creerMap() {
		...
		if (this.mapLargeur >= this.mapHauteur) {
			...
			this.panelMap.setLayout(new GridLayout(this.mapLargeur,
					this.mapLargeur));
			...
			this.panelMap.setLayout(new GridLayout(this.mapHauteur,
					this.mapHauteur));
	...
	public void initCl(String aAfficher, boolean peutmodifier) {
		...
		if (peutmodifier) {
			String[] boutonpossible = { "archer", "cavalier archer",
					"détruire", "catapultes", "détruire", "milicien",
					"détruire", "chevalier", "détruire", "peon", "détruire",
					"détruire", "galère", "tranporteur", "détruire",
					"détruire", "magicien", "healers", "détruire", "détruire",
					"déplacer", "attaquer", "détruire", "déplacer", "attaquer",
					"détruire", "déplacer", "attaquer", "détruire", "déplacer",
					"attaquer", "détruire", "déplacer", "attaquer", "détruire",
					"déplacer", "soigner", "détruire", "déplacer", "attaquer",
					"détruire", "déplacer", "attaquer", "détruire", "Archerie",
					"atelier de siège", "Caserne", "Ecurie", "Hotel de ville",
					"Mur", "Port", "mine or", "mine nourriture", "mine bois",
					"mine fer", "mine pierre", "Tour des mages", "Tourelle",
					"déplacer", "rechercher", "détruire", "déplacer",
					"attaquer", "détruire" };
			...
			for (int i = 0; i < boutonpossible.length; i++) {
				...
				if (boutonpossible[i] == "détruire") {
					...
					if (j >= this.jcard.length) {
						System.out
								.println("problème lors de l'initialisation du card layout du panel bas");
					}
				}
			}
			if (actionEnCours) {
				JLabel quoifaire = new JLabel(
						"selectionner une case où executer l'action en cours");
			...
			for (i = 0; i < this.mapLargeur; i++) {
				for (j = 0; j < this.mapLargeur; j++) {
					if (i < test2 || i >= (this.mapHauteur + test2)) {
						...
						this.map[i][j]
								.setBackground(this.saFenetre.affichagejeu
										.getCarte(i - test2, j).getBackground());
					}
				}
			}
		} else {
			...
			for (i = 0; i < this.mapHauteur; i++) {
				for (j = 0; j < this.mapHauteur; j++) {
					if (j < test2 || j >= (this.mapLargeur + test2)) {
						...
						this.map[i][j]
								.setBackground(this.saFenetre.affichagejeu
										.getCarte(i, j - test2).getBackground());
					}
				}
			}
		}
	...
	public void updateActionPossible(Case bouh, int hauteur, int largeur) {
		...
		if (!actionEnCours) {
			if (bouh.isBatimentsurcase()) {
				Batiment batimentSurLaCase = (Batiment) recuperer(hauteur,
						largeur, true);
				...
				Personnage personneSurLaCase = (Personnage) recuperer(hauteur,
						largeur, false);
		...
		if (wantBatiment) {
			for (Joueur ceJoueur : this.saFenetre.lesJoueurs) {
				...
				for (Batiment celuiLa : batimentPossible) {
					if (celuiLa.getPOSITION(0) == largeur
							&& celuiLa.getPOSITION(1) == hauteur) {
						return celuiLa;
					}
				}
			}
			...
			for (Joueur ceJoueur : this.saFenetre.lesJoueurs) {
				ArrayList<Personnage> personnagePossible = ceJoueur
						.getPersonnages();
				for (Personnage celuiLa : personnagePossible) {
					if (celuiLa.getPositionHorizontale() == largeur
							&& celuiLa.getPositionVerticale() == hauteur) {
						return celuiLa;
					}
				}
			}
	...
	public void executerActionComplexe(int positionHauteur, int positionLargeur) {
		...
		case "attaquer":
			break;
		case "déplacer":
			Evenements.Boutons.deplace(
					(Personnage) this.trucActuellementSelectionne,
					this.positionDeLaCaseActuellementSelectionnee[1],
					this.positionDeLaCaseActuellementSelectionnee[0],
					this.saFenetre.affichagejeu.getCarte(
							this.positionDeLaCaseActuellementSelectionnee[0],
							this.positionDeLaCaseActuellementSelectionnee[1]),
		...
		default:
			System.out
					.println("problème lors de la recuperation de l'action complexe à effectuer");
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "chevalier":
			break;
		case "peon":
			if (this.saFenetre.lesJoueurs[PanelResrc.joueurencours]
					.isPeonCreated(this.positionDeLaCaseActuellementSelectionnee)) {
				System.out.println("Unité construite");
			} else {
				System.out
						.println("Pas assez de ressources ou plus de place autour du batiment");
			}
			break;
		case "galère":
			break;
		...
		case "Archerie":
			if (this.saFenetre.lesJoueurs[PanelResrc.joueurencours]
					.isArcherieConstructed(
							this.positionDeLaCaseActuellementSelectionnee,
							this.saFenetre.affichagejeu
									.getCarte(
											this.positionDeLaCaseActuellementSelectionnee[1],
											this.positionDeLaCaseActuellementSelectionnee[0]))) {
		...
		default:
			System.out
					.println("problème lors de la recuperation de l'action à affectuer");
		}
		this.saFenetre.updateAffichage();
	}
}
